...
@php
    $status = \App\Helpers\FMF::getNextStatus($staticModuleId, $prasaranaSukan->status, 'success');
    $reject = \App\Helpers\FMF::getNextStatus($staticModuleId, $prasaranaSukan->status, 'reject');
    $query = \App\Helpers\FMF::getNextStatus($staticModuleId, $prasaranaSukan->status, 'query');
@endphp
<div class="modal-footer">
    @if($canVerify && !$canApprove)
        <button type="button" class="btn btn-primary" style="margin-right:10px;" onclick="formverifyIkeps({{$status}},{{$prasaranaSukan->id}})">Verify</button>
    @endif
    @if($canApprove)
        <button type="button" class="btn btn-primary" style="margin-right:10px;" onclick="formverifyIkeps({{$status}},{{$prasaranaSukan->id}})">Approve</button>
    @endif
    @if($canQuery)
        <button type="button" class="btn btn-primary" style="margin-right:10px;" onclick="formverifyIkeps({{$query}},{{$prasaranaSukan->id}})">Query</button>
    @endif
    
    <button type="button" class="btn btn-danger" style="margin-right:10px;" onclick="formverifyIkeps({{$reject}},{{$prasaranaSukan->id}})">Reject</button>
</div>
<div>
    &nbsp;
</div>
