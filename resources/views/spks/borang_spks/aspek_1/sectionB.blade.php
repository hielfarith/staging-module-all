...
<?php
if ($spks) {
    $aspek1_sectionb = json_decode($spks->aspek1_sectionb, true);
} else {
    $aspek1_sectionb = null;
}
?>
<form id="aspek1_sectionb">
    ...
    <div class="table-responsive">
        <table class="table header_uppercase table-bordered table-hovered" id="spks_aspek1">
            ...
            <tbody>
                ...
                @foreach ($aspeks_1_secB as $index => $aspek_1_secB)
                    <tr>
                        <td colspan="6" class="bg-light-primary text-uppercase">
                            {{ $aspek_1_secB['section'] }}
                        </td>
                    </tr>
                    @foreach ($aspek_1_secB['subSections'] as $subsection_aspek1)
                        <?php
                        
                        $name = $index . '_' . $loop->index;
                        $withTB = $aspek_1_secB['section'] == 'Arahan Keselamatan Murid Dari Aspek Pergi Dan Balik Sekolah' || $aspek_1_secB['section'] == 'Arahan Keselamatan Murid Semasa Pengajaran & Pembelajaran Dan Waktu Rehat' || $aspek_1_secB['section'] == 'Arahan Keselamatan Murid Semasa Aktiviti Kokurikulum, Sukan Dan Permainan' || $aspek_1_secB['section'] == 'Arahan Keselamatan Murid Semasa Aktiviti Lawatan Dan Perkhemahan' || $aspek_1_secB['section'] == 'Arahan Keselamatan Murid Di Asrama';
                        $nameIndex = $index . '_' . $loop->index;
                        $catatanIndex = 'catatan_' . $nameIndex;
                        $catatan = '';
                        if ($aspek1_sectionb) {
                            $catatan = isset($aspek1_sectionb[$catatanIndex]) ? $aspek1_sectionb[$catatanIndex] : '';
                        }
                        ?>
                        <tr>
                            <td>{{ $number++ }}</td>
                            ...
                            <td>
                                <div class="d-flex justify-content-center align-items-center">
                                    <input required class="form-check-input radio-input-2" type="radio"
                                        ...
                                        @if (isset($aspek1_sectionb) && $aspek1_sectionb[$nameIndex] == '0') checked @endif {{ $disabled }}>
                                </div>
                            </td>
                            <td>
                                <div class="d-flex justify-content-center align-items-center">
                                    <input required class="form-check-input radio-input-2" type="radio"
                                        ...
                                        @if (isset($aspek1_sectionb) && $aspek1_sectionb[$nameIndex] == '1') checked @endif {{ $disabled }}>
                                </div>
                            </td>
                            <td>
                                <div class="d-flex justify-content-center align-items-center">
                                    <input required class="form-check-input radio-input-2" type="radio"
                                        ...
                                        @if (isset($aspek1_sectionb) && $aspek1_sectionb[$nameIndex] == '2') checked @endif {{ $disabled }}>
                                </div>
                            </td>
                            <!--  @if ($withTB)
<td>
...
@endif -->
                        ...
                        <tr>
                            <td colspan="6" class="bg-light-success">
                                <input required type="text" name="catatan_{{ $nameIndex }}" class="form-control"
    ...
    @if ($disabled != 'disabled')
        <div class="buy-now">
            <button class="btn btn-primary waves-effect waves-float waves-light" type="button"
                onclick="formsubmit('aspek1_sectionb')">
                Simpan
            </button>
        </div>
    @endif
