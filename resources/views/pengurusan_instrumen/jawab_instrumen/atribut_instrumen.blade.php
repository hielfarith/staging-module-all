@if($insertone)
	@if( in_array($array['type'], ['text', 'number']))
		<x-input-field class="form-control" type="{{ $array['type'] }}" name="{{ $array['name'] }}" value="" label="{{ $array['label'] }}" />
	...
	@elseif($array['type'] == 'file')
		<x-input-file-field class="form-control" name="{{ $array['name'] }}" label="{{ $array['label'] }}" accept=".pdf,.doc,.docx">
		</x-input-file-field>
	@endif
@else
    ...
    <form id="borang_jawab_instrumen">
        ...
        @foreach($arrays as $array)
            @if( in_array($array['type'], ['text', 'number']))
                ...
                @else
                    <x-input-field type="{{ $array['type'] }}" name="{{ $array['name'] }}" value="" label="{{ $array['label'] }}" required/>
                @endif
            @elseif($array['type'] == 'select')
                <x-input-select-field class="form-control select2" name="{{ $array['name'] }}" label="{{ $array['label'] }}" required>
                    <option value="" hidden>Pilih {{ $array['label'] }}</option>
            ...
            @elseif($array['type'] == 'file')
                <x-input-file-field class="form-control" name="{{ $array['name'] }}" label="{{ $array['label'] }}" accept=".pdf,.doc,.docx" required>
                </x-input-file-field>
            @endif
        @endforeach
