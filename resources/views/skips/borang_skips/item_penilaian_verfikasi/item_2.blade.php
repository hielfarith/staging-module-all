                        ...
                        @foreach ($option_institusis[$index] as $key => $option_institusi)
                            <td>
                                ...
                                <div class="form-check form-check-inline d-flex justify-content-center align-items-center">
                                    <input class="form-check-input" type="radio" name="{{ $index }}_verfikasi" id="{{ $index }}" value="{{$key}}" required @if($pengurusan_institusi && $pengurusan_institusi->$keyval == $key) checked @endif @if($type == 'validasi'  || $status == 'done') disabled @endif>
                                </div>
                                @endif
                                <br>
...
<script>
   function submitform2v() {
        ...
        var url = "{{ env('APP_VERFIKASI_URL') }}" + 'api/skips/verfikasi/submit/pengurusan_institusi_verfikasi';
        $.ajax({
            url: url,
            method: 'POST',
            data: formData,
