                        ...
                        @foreach ($option_kurikulums[$index] as $key => $option_kurikulum)
                            <td>
                                <div class="form-check form-check-inline d-flex justify-content-center align-items-center">
                                    <input class="form-check-input" type="radio" name="{{ $index }}_verfikasi" id="" value="{{$key}}" @if($pengurusan_kurikulum && $pengurusan_kurikulum->$keyval == $key) checked @endif @if($type == 'validasi' || $status == 'done') disabled @endif>
                                </div>
                                <br>
...
<script>
   function submitform3v() {
        ...
        var url = "{{ env('APP_VERFIKASI_URL') }}" + 'api/skips/verfikasi/submit/pengurusan_kurikulum_verfikasi';
